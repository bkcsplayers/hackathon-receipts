...
from services.processing_tracker import add_llm_usage, complete_job, fail_job, update_step
...
async def process_receipt_upload(
    ...
    async def step(n: int, message: str):
        if job_id:
            await update_step(db_session, job_id, n, message)
        if progress_callback:
            await progress_callback(step=n, message=message)

    try:
        await step(0, "Compressing image...")
        ...
        await step(1, "Uploading to cloud...")
        original_url, webp_url = await upload_receipt_files(user_id, file_bytes, webp_bytes, filename)

        await step(2, "AI is scanning receipt...")
        ocr_text, ocr_usage = await ocr_receipt_image(webp_bytes, OCR_SYSTEM_PROMPT)
        if job_id:
            await add_llm_usage(db_session, job_id, ocr_usage)
        logger.info("ocr_completed", text_length=len(ocr_text))

        await step(3, "Extracting data...")
        extracted, extract_usage = await extract_receipt_data(ocr_text, EXTRACTION_SYSTEM_PROMPT)
        if job_id:
            await add_llm_usage(db_session, job_id, extract_usage)
        ...
        await step(6, "Manifested! ✅")
        if job_id:
            await complete_job(db_session, job_id, receipt.id, started_monotonic=started)
            await db_session.commit()
        ...
        if job_id:
            await fail_job(db_session, job_id, str(exc), started_monotonic=started)
            await db_session.commit()
        raise
